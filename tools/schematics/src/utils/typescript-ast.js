...
exports.getTypeScriptSourceFile = exports.variablePush = exports.objectPush = exports.classMethodPush = exports.arrayUnshift = exports.importModuleInFile = exports.findGenericNode = exports.findPropertyNode = exports.findClassDeclarationNode = exports.findVariableDeclarationNode = exports.findVariableNode = void 0;
...
function findVariableNode(source, name) {
    const nodes = (0, ast_utils_1.findNodes)(source, ts.SyntaxKind.VariableStatement);
    return (nodes.find(node => node.declarationList.declarations[0].name.getText() === name) || null);
}
...
function importModuleInFile(source, barrelIndexPath, symbolName, fileName, isDefault = false) {
    const changes = [];
    const nodes = (0, ast_utils_1.findNodes)(source, ts.SyntaxKind.ImportDeclaration);
    if (!nodes.find(node => node.moduleSpecifier.getText() === `'${fileName}'`)) {
        ...
        changes.push(new change_1.InsertChange(barrelIndexPath, lastItemPosition, `\nimport ${open}${symbolName}${close} from '${fileName}';`));
    }
    return changes;
}
...
function arrayUnshift(node, barrelIndexPath, toAdd) {
    ...
    const changes = [];
    if (!initializer)
        throw new angularDevkit.SchematicsException(`Node '${declaration.name.getText()}' is not initialized`);
    if (!ts.isArrayLiteralExpression(initializer))
        throw new angularDevkit.SchematicsException(`Node '${declaration.name.getText()}' is not of type array`);
    if (initializer.elements.length === 0) {
        if (initializer.getWidth() > 2) {
            changes.push(new change_1.RemoveChange(barrelIndexPath, initializer.getStart() + 2, ' '.repeat(initializer.getWidth() - 3)));
        }
        changes.push(new change_1.InsertChange(barrelIndexPath, initializer.getStart() + 1, `\n${const_1.FILE_INDENT}${toAdd}\n`));
    }
    else {
        changes.push(new change_1.InsertChange(barrelIndexPath, initializer.getStart() + 1, `\n${const_1.FILE_INDENT}${toAdd},`));
    }
    ...
    node.members.forEach(member => {
        if (member.name) {
            if (member.name.getText() === methodToAdd) {
                throw new angularDevkit.SchematicsException(`Method '${methodToAdd}' already existing in class`);
            }
        }
    });
    changes.push(new change_1.InsertChange(barrelIndexPath, node.getEnd() - 1, `\n${const_1.FILE_INDENT}${toAdd}\n`));
    return changes;
}
...
function objectPush(node, barrelIndexPath, toAdd) {
    ...
    const changes = [];
    if (!initializer)
        throw new angularDevkit.SchematicsException(`Node '${node.name.getText()}' is not initialized`);
    if (!ts.isObjectLiteralExpression(initializer))
        throw new angularDevkit.SchematicsException(`Node '${node.name.getText()}' is not of type object`);
    changes.push(new change_1.InsertChange(barrelIndexPath, initializer.getEnd() - 4, `\n${const_1.FILE_INDENT}${const_1.FILE_INDENT}${toAdd},`));
    return changes;
}
...
const getTypeScriptSourceFile = (host, path) => ts.createSourceFile(path, (0, workspace_1.readFile)(path)(host).toString('utf8'), ts.ScriptTarget.Latest, true);
