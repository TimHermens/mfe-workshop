...
function insertConfigIntoFeaturesModule(tree, configToAdd) {
  const globalSitesAppPath = (0, workspace_1.getProjectSourceRoot)(
    tree,
    'global-sites'
  );
  const featureModulePath = `${globalSitesAppPath}/app/config/feature-modules.config.ts`;
  const featureModuleSourceFile = (0, typescript_ast_1.getTypeScriptSourceFile)(
    tree,
    featureModulePath
  );
  ...
  const variableDeclaration = (0, typescript_ast_1.findVariableDeclarationNode)(
