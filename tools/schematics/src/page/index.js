...
async function default_1(tree, schema) {
  try {
    ...
    if (missingProperties.length > 0) {
      throw new Error(
        `Required properties ${missingProperties
          .map((p) => `'${p}'`)
          .join(', ')} are undefined`
      );
    }
    ...
    const options = {
      ...
      pagePathAlias: `@${(0, workspace_1.getWorkspaceName)(tree)}/${
        schema.domain
      ...
      selector: schema.selector
        ? schema.selector
        : `${prefix}-${pageNames.fileName}`,
      pageProjectRoot: (0, devkit_1.joinPathFragments)(
        parentPageRootPath,
        `${prefix}-${pageNames.fileName}`,
        '_index'
      ),
    ...
    const projectConfiguration = {
      ...
      targets: {
        build: {
          ...
          outputs: [
            (0, devkit_1.joinPathFragments)('dist', options.pageProjectRoot),
          ],
          options: {
            project: (0, devkit_1.joinPathFragments)(
          ...
          configurations: {
            production: {
              tsConfig: (0, devkit_1.joinPathFragments)(
                options.pageProjectRoot,
    ...
    (0, devkit_1.addProjectConfiguration)(
      tree,
    ...
    (0, devkit_1.generateFiles)(
      tree,
      (0, devkit_1.joinPathFragments)(__dirname, 'files', 'default'),
      options.pageProjectRoot,
      {
        ...schema,
        name: pageNames.fileName,
...
function addDefaultPageRoute(tree, options, parentPageRootPath, pageNames) {
  ...
  if (options.hasParentPage) {
    const parentPageName = parentPageRootPath.match(
      new RegExp(/\/page-(.*)$/m)
    )[1];
    routingModulePath = (0, devkit_1.joinPathFragments)(
      parentPageRootPath,
      '_index',
  ...
  const routingModuleSource = (0, typescript_1.createSourceFile)(
    routingModulePath,
  ...
  (0, angular_ast_utils_1.addRoute)(
    tree,
    routingModulePath,
    routingModuleSource,
    route
...
const getParentRouteNamePrefix = (parentRootPath, options) => {
  if (options.hasParentPage) {
    return (0, devkit_1.names)(parentRootPath.split('/').slice(-2)[0])
      .className;
  }
  return 'app';
};
